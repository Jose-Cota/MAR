...
try {
    ...
    $stmt = $pdo->query("SELECT proyecto_id, nombre FROM proyectos WHERE ejercicio_id = 19 AND (nombre LIKE '%Acciones del Pleno%' OR nombre LIKE '%Acciones que la Ponencia%')");
    ...
    foreach ($proys as $p) {
        ...
        echo "POA ID " . $p['proyecto_id'] . " | Acts: " . $stmt2->fetchColumn() . " | " . $p['nombre'] . "\n";
    }
