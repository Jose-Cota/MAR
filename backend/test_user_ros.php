...
$user = User::where('usuario', 'usuario_varias')->first(); 
if(!$user) {
    echo "Creating dummy user for test...\n";
    // just pick some user with multiple ROs
    $user = DB::connection('poa_prod')->table('usuarios_poa')->whereIn('usuario_poa_id', function($q){
        $q->select('usuario_poa_id')->from('usuarios_responsables_operativos')
          ->groupBy('usuario_poa_id')->havingRaw('COUNT(*) > 1');
    })->first();
    
    if(!$user) {
        echo "No users with multiple ROs found.\n";
    } else {
