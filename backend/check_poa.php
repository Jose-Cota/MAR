...
$py = DB::connection('poa_prod')->select("
    SELECT py.proyecto_id, py.numero, py.nombre, py.status, urg.numero as urg_num, p.numero as p_num, sp.numero as sp_num 
    FROM proyectos py 
...
foreach($py as $p) { 
    echo "ID: {$p->proyecto_id} | U:{$p->urg_num} P:{$p->p_num} S:{$p->sp_num} N:{$p->numero} | Nombre: {$p->nombre}\n";
}
